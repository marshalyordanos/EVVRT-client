  ...
  async (data, { rejectWithValue, getState }) => {
    try {
      const { searchText, page, limit, sort, order } =
        getState().indicators.query; // Access state directly

      const res = await IndicatorsService.searchIndicator({
        page,
        limit,
        searchText,
        sort,
...
export const indicatorsSearchText = (state) =>
  state.indicators.query.searchText;
